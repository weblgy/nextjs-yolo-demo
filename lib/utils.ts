...
// 1. 在最上面加一个接口，定义我们要返回的数据结构
export interface Detection {
  label: string;
  box: [number, number, number, number]; // 包含坐标信息
}
...
export function renderBoxes(
  ...
  // 🔥 新增：画一个高科技感的红色虚线电子围栏 (占据画面中间 60% 的区域)
  const fenceX = canvas.width * 0.2;
  const fenceY = canvas.height * 0.2;
  const fenceW = canvas.width * 0.6;
  const fenceH = canvas.height * 0.6;

  ctx.strokeStyle = "rgba(255, 0, 0, 0.5)"; // 半透明红色
  ctx.lineWidth = 4;
  ctx.setLineDash([15, 10]); // 虚线效果

  ctx.setLineDash([]); // 恢复实线，以免影响后面画 YOLO 的框
  ...
  return detectedItems; // <--- 新增：在函数最后，把数组返回出去
}